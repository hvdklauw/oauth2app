...
class AccessRange(models.Model):
    ...
    * *description:* A string representing the access range description.
    ...
    class Meta:
        verbose_name = _('access range')
        verbose_name_plural = _('access ranges')

    def __unicode__(self):
        return u"{0} ({1})".format(self.key, self.description)
...
class AccessToken(models.Model):
    ...
    * *refreshable:* A boolean that indicates whether this access token is
    ...
    def __unicode__(self):
        return u"{0} ({1})".format(self.client, self.token)
...
class Code(models.Model):
    ...
    * *scope:* A list of oauth2app.models.AccessRange objects. *Default None*
    ...
    def __unicode__(self):
        return u"{0} ({1})".format(self.client, self.key)
...
class MACNonce(models.Model):
    ...
    * *nonce:* A unique nonce string.
    ...
    class Meta:
        verbose_name = _('MAC nonce')
        verbose_name_plural = _('MAC nonces')

    def __unicode__(self):
        return u"{0} ({1})".format(self.access_token, self.nonce)
